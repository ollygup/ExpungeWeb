...
export class FaqComponent {
  ...
  readonly nerdFaqs: FaqItem[] = [
    ...
    },
    {
      ...
      a: 'There are two modes depending on what you\'re redacting. For normal text, it uses text-based searching directly against the PDF content streams — MuPDF (compiled to WASM, running in a Web Worker) finds the matching text spans and removes those operators from the stream entirely. For images, it uses Scribe OCR to detect text within the image and redact the region. Scribe can technically find normal text too, but text-based comparison will always be more accurate for that use case so both are kept separate. Which results in a true redaction — not a visual overlay, not a black box on top. The content is gone from the file structure. Try selecting or copy-pasting the redacted area yourself, nothing there? Good, that\'s the point :) Works on most standard PDFs (Word, Google Docs, LibreOffice exports). Heavily encoded or scanned-only PDFs may vary though',
    },
  ];
